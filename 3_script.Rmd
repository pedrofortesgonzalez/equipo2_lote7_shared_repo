...
#### Loadings
Para un análisis más detallado de todo esto, la propia función prcomp() devuelve un df que refleja cuánto contribuye cada gen a cada PC, así que los guardo en una tabla (**Tabla 2**).
```{r}
...
gtsave(gt(pca_loadings_exp), filename = "4_tables/Tabla 2 - PCA Cargas.html") # guardar html
```
...
Objetivo: hacer una tabla donde se recojan medidas descriptivas de cada tercil de las 5 primeras PCs. Para ello, antes debemos chequear la normalidad para ver qué test haremos entre las medias/medianas de los 3-iles.

### Chequear normalidad
Como en la tabla incluiremos test de contraste de hipótesis para comparar los 3-iles, debemos elegir el test antes de construirla. Testearemos la igualdad de expresión para cada gen a lo largo de los terciles de cada componente del PCA, es decir, que testearemos la igualdad de medias (e.paramétrica) o medianas (e. no paramétrica) de 3 grupos (los terciles del PC en cuestión). Tenemos, por tanto, que calcular la normalidad para las variables de expresión génica, ya que usaremos:

- v. con d. normal: one way ANOVA
- v. con d. no normal: test de Kruskal-Wallis

Elegimos estos test porque las muestras no son emparejadas. Por tanto, calculamos normalidad en df_genes y almacenamos vars. con d. normal en un vector y aquellas con d. no normal en otro:
```{r}
...
Todos los genes tienen una distribución no normal, por lo que testearemos las diferencias entre sus terciles con el *test de Kruskal-Wallis*.
...
Tenemos demasiadas variables independientes por lo que, antes de aplicar la regresión logística, decidimos usar un modelo de regularización, para reducir el número de variables. Al principio, intentamos usar LASSO, un modelo de regularización que aplica una penalización sobre las variables y permite seleccionar solo aquellas variables con un coeficiente que tras la penalización tengan un valor mayor a 0, que son las variables más importantes a la hora de hacer el análisis. Sin embargo, este método salía mal ya que este tipo de regularización eliminaba todas las variables.
...
Lo que vemos es que el lambda.min (el que minimiza la diferencia $V_{real}-V_{predicha}$) me dejará solo una variable (intercepto), pero podemos usar otra regla: en vez de el lambda mínimo, usamos el lambda "within one standard error from the minimum". Este es el lambda que minimiza el CV error + 1 SE (si no, el valor de lambda mínimo hace se eliminen todas las variables). 

Generamos el nº y lo uso para generar modelo de Lasso.
```{r}
...
Este lambda también me elimina todas las variables, ya que en realidad es muy cercano al `lambda_min`, lo cual comprobamos a continuación
```{r}
...
Ahora que hemos encontrado un alpha intermedio entre Lasso y Ridge, vamos a probar a utilizarlo con `glmnet()` para encontrar el lambda óptimo.
...
lambda_1se
```
En el plot vemos que el lambda mínimo produciría otra vez la eliminación de todas las variables. Sin embargo, en el rango de valores de su SE podemos ver que hay algunos valore de lambda (a partir de $10^{-2}$) donde sí se conservan algunas variables. Viendo también que $\lambda_{+1S.E.}=1=10^{0}$, podremos deducir que esta medida no es buena, por ello hemos escogido arbitrariamente medidas más pequeñas de lambda, para reducir la regularización y conservar ciertas variables que puedan aportar valor predictivo a nuestro modelo. Empezamos con $\lambda=10^{-1}$.
...
Las variables sociodemográficas no salen significativas, por lo que pueden aportar ruido al modelo. Ajustamos un último modelo solo con las PCs.
### modelo RL 3
```{r}
# Eliminar las variables específicas de la fórmula
...
# PENDIENTE
- columna Var: renombrar como columna tercil y as.factor()
-- Intercept = t1
-- para cada var:
---- TPCxt2 = t2
---- TPCxt3 = t3

- añadir pvalue como nueva columna a la derecha de coeficientes --> están en rl_summary_2$coeftable

- añadir nuevas columnas con IC 95% --> están en rl_summary_2$coeftable

- una vez hecho esto, crear última tabla

- en gráficas de `### B-Analiza variables y PCs`, en la gráfica que son 4 en 1 y se ve mal, dejar comentario explicando que en .html se verá mal pero que si la abren en Rmd (y la visualizan en el popup), se verá mejor ya que el tamaño se autoajusta