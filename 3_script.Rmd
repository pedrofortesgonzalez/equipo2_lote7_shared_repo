...
Primeramente, se testearon con la función glmnet() diferentes valores de lambda estabelcidos previamente en la variable grid. Al graficar estos valores de lambda, pudimos orientar entorno a qué rango de valores se encontraba el lambda óptimo. 
...
library(broom)
library(stringr)
# Asumiendo que 'modelo' es tu objeto de glm
df_ors <- as.data.frame(tidy(modelo_regresion3, exponentiate = TRUE, conf.int=TRUE))  # Esto te da un dataframe con los términos, estimaciones, etc.
# separar df_ors$term en PC y Tercil
matches <- stringr::str_match(df_ors$term, "^(?:T)(PC\\d)(t\\d)$")
df_ors[c("PC", "Tercil")] <- matches[,2:3]
...
df_wide$PC = c("PC 1", "PC 2", "PC 3", "PC 4", "PC 5") 
df_wide <- df_wide %>% dplyr::select("PC", 
              "OR t1", "p.value t1", 
              "OR t2", "p.value t2", 
              "OR t3", "p.value t3")
...
```{r}
tidy(modelo_regresion3)
glance(modelo_regresion3)
augment(modelo_regresion3)
```


## Pedro: t3-iles ORs
En esta versión, si fuésemos capaces de añadir los intervalos de confianza y pvalores, podríamos usar `tbl_regression()` pero veo problemas con el formato

Documentation: https://www.danieldsjoberg.com/gtsummary/articles/tbl_regression.html
https://www.danieldsjoberg.com/gtsummary/reference/tbl_regression.html 
https://www.rdocumentation.org/packages/gtsummary/versions/1.2.0/topics/tbl_regression 
https://www.danieldsjoberg.com/gtsummary-weill-cornell-presentation/#38 
...
rl_summary_3 <- summary(modelo_regresion_3)
rl_summary_3$coeftable#aquí están los pvalues y los IC95%
```
...
OR_d_table = OR_d_table %>% dplyr::rename(Tercil = Var)
OR_d_table$Tercil = as.factor(OR_d_table$Tercil)
levels(OR_d_table$Tercil) = list(
  T1 = "(Intercept)", 
  T2 = c("TPC1t2", "TPC2t2", "TPC3t2", "TPC4t2", "TPC5t2"), 
  T3 = c("TPC1t3", "TPC2t3", "TPC3t3", "TPC4t3", "TPC5t3"))
gt(OR_d_table)
```

## Cris
Aqui estaba probando cosas, pero no me sale nada con sentido
```{r}

#A mi se me habia ocurrido esto, pero hay que pulirlo 
...
ICs <- as.data.frame(exp(confint(modelo_regresion3))) #esto para añadir los ICs, pero aun sigo pensando como
